...
appearanceClassifier = classifier.AppearanceClassifier.load(args.classifierFilename)
pastObjects = []
currentObjects = []
if capture.isOpened():
    ret = True
        ...
        if ret:
            ...
            for obj in currentObjects:
                if obj.getLastInstant() <= frameNum:  # if images are skipped
                    appearanceClassifier.classify(obj)
                    pastObjects.append(obj)
                    currentObjects.remove(obj)
                else:
                    appearanceClassifier.classifyAtInstant(obj, img, frameNum, width, height)
        frameNum += 1
    
    for obj in currentObjects:
        appearanceClassifier.classify(obj)
        pastObjects.append(obj)
    #appearanceClassifier.save("saveAppearance.yaml")
